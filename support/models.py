...
from support.choices import (
    ISSUE_CATEGORIES, ISSUE_ANSWERS, ISSUE_STATUS, ISSUE_SUBCATEGORIES, SCHEDULED_TASK_CATEGORIES
)
...
class Seller(models.Model):
    ...
    def get_unfinished_campaigns(self):
        seller_campaigns = Campaign.objects.filter(
            ...
            contactcampaignstatus__status__lt=4).distinct()
        return seller_campaigns

    def get_campaigns_by_status(self, status):
        seller_campaigns = Campaign.objects.filter(
            contactcampaignstatus__contact__seller=self,
            contactcampaignstatus__status__in=status).distinct()
        return seller_campaigns
...
class Issue(models.Model):
    ...
    address_1 = models.ForeignKey('core.Address', blank=True, null=True, related_name='issue_address_1')
    address_2 = models.ForeignKey('core.Address', blank=True, null=True, related_name='issue_address_2')
    route = models.ForeignKey('logistics.Route', blank=True, null=True)  # Only for logistics
    manager = models.ForeignKey(
        'auth.User', blank=True, null=True, related_name='issue_manager')  # User who created the issue. Non-editable
    assigned_to = models.ForeignKey(
        'auth.User', blank=True, null=True, related_name='issue_assigned')  # Editable, assigned to which user
    ...
    status = models.CharField(max_length=1, blank=True, null=True, choices=ISSUE_STATUS, default='P')
    ...
    # Optional attributes
    subscription_product = models.ForeignKey('core.SubscriptionProduct', null=True, blank=True)
