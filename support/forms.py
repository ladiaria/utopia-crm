...
from core.models import Contact, Product, Subscription, Address
...
class ServiceIssueStartForm(forms.ModelForm):
    ...
    class Meta:
        model = Issue
        fields = (
            'contact', 'category', 'subcategory', 'notes', 'subscription')


class NewPauseScheduledTaskForm(forms.Form):
    subscription = forms.ModelChoiceField(
        Subscription.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
    date_1 = forms.DateField(widget=forms.DateTimeInput(attrs={'class': 'datepicker form-control float-right'}))
    date_2 = forms.DateField(widget=forms.DateTimeInput(attrs={'class': 'datepicker form-control float-right'}))
...
class NewAddressChangeScheduledTaskForm(forms.Form):
    ...
    contact_address = forms.ModelChoiceField(
        Address.objects.all(), required=False, widget=forms.Select(attrs={'class': 'form-control'}))
    ...
    new_address_type = forms.ChoiceField(
        required=False, choices=ADDRESS_TYPE_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))
...
class NewPromoForm(forms.Form):
    ...
    start_date = forms.DateField(
        widget=forms.DateInput(format='%Y-%m-%d', attrs={'class': 'datepicker form-control'}))
...
class NewSubscriptionForm(forms.Form):
    ...
    frequency = forms.ChoiceField(
        required=False, choices=FREQUENCY_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))
    payment_type = forms.ChoiceField(
        required=False, choices=settings.SUBSCRIPTION_PAYMENT_METHODS,
...
class GestionStartForm(forms.ModelForm):
    ...
    class Meta:
        model = Issue
        fields = (
            'contact', 'date', 'category', 'subcategory', 'notes', 'assigned_to', 'progress', 'answer_1', 'answer_2',
            'status', 'end_date', 'next_action_date', 'closing_date', 'subscription')
...
class LogisticsIssueStartForm(forms.ModelForm):
    ...
    contact = forms.ModelChoiceField(
        queryset=Contact.objects, widget=forms.TextInput(attrs={'class': 'form-control'}))

    widget = forms.Select(attrs={'class': 'form-control'})
    # contact.disabled = True
    product = forms.ModelChoiceField(
        queryset=Product.objects.filter(type='S'), widget=forms.Select(attrs={'class': 'form-control'}))
    ...
    subcategory = forms.ChoiceField(
        choices=logistics_subcategories, widget=forms.Select(attrs={'class': 'form-control'}))
    ...
    class Meta:
        ...
        fields = (
            'contact', 'category', 'subcategory', 'notes', 'copies', 'subscription_product',
...
class LogisticsIssueChangeForm(forms.ModelForm):
    ...
    product = forms.ModelChoiceField(queryset=Product.objects.filter(type='S', bundle_product=False))
    # Add a default empty option
    logistics_subcategories = [('', '-----')]
    for subcategory in ISSUE_SUBCATEGORIES:
        # Only show the options that are set as logistics, they all start with an L on the key
    ...
    class Meta:
        ...
        fields = (
            'contact', 'date', 'category', 'subcategory', 'notes', 'assigned_to', 'progress', 'answer_1', 'answer_2',
            'status', 'end_date', 'next_action_date', 'closing_date', 'subscription', 'product', 'copies',
...
class SelectContactForIssue(forms.Form):
    contact_id = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control'}))
    issue_type = forms.ChoiceField(
        choices=SELECT_CONTACT_FOR_ISSUE_TYPE_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))
