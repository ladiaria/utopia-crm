...
class ValidationError(Exception):
    def __init__(self, message):
        ...
        if isinstance(message, list):
            ...
            assert isinstance(message, (str, Promise)), ("%s should be a string" % repr(message))
            self.messages = [message]
...
class CriticalValidationError(Exception):
    def __init__(self, message):
        ...
        if isinstance(message, list):
            ...
            assert isinstance(message, (str, Promise)), ("'%s' should be a string" % message)
            self.messages = [message]
...
def isValidHTML(field_data, all_data):
    ...
    try:
        u = urlopen('http://validator.w3.org/check', urlencode({'fragment': field_data, 'output': 'xml'}))
    except Exception:
    ...
    error_messages = [
        e.firstChild.wholeText
        for e in parseString(u.read()).getElementsByTagName('messages')[0].getElementsByTagName('msg')
    ]
...
def isWellFormedXml(field_data, all_data):
    ...
    except Exception as e:  # Naked except because we're not sure what will be thrown
...
class AlwaysMatchesOtherField:
    def __init__(self, other_field_name, error_message=None):
        ...
        self.error_message = error_message or lazy_inter(
            _("This field must match the '%s' field."), self.other
        )
...
class RequiredIfOtherFieldNotGiven:
    def __init__(self, other_field_name, error_message=_("Please enter something for at least one field.")):
...
class RequiredIfOtherFieldsGiven:
    def __init__(self, other_field_names, error_message=_("Please enter both fields or leave them both empty.")):
...
class RequiredIfOtherFieldGiven(RequiredIfOtherFieldsGiven):
    ...
    def __init__(self, other_field_name, error_message=_("Please enter both fields or leave them both empty.")):
...
class RequiredIfOtherFieldEquals:
    def __init__(self, other_field, other_value, error_message=None):
        ...
        self.error_message = error_message or lazy_inter(_("This field must be given if %(field)s is %(value)s"), {
...
class RequiredIfOtherFieldDoesNotEqual:
    def __init__(self, other_field, other_value, error_message=None):
        self.other_field = other_field
        self.other_value = other_value
        self.error_message = error_message or lazy_inter(_("This field must be given if %(field)s is not %(value)s"), {
            'field': other_field, 'value': other_value})
        self.always_test = True
...
class UniqueAmongstFieldsWithPrefix:
    def __init__(self, field_name, prefix, error_message):
        ...
        self.error_message = error_message or _("Duplicate values are not allowed.")

    def __call__(self, field_data, all_data):
        for field_name, value in list(all_data.items()):
...
class HasAllowableSize:
    ...
    def __init__(self, min_size=None, max_size=None, min_error_message=None, max_error_message=None):
        ...
        self.min_error_message = \
            min_error_message or lazy_inter(_("Make sure your uploaded file is at least %s bytes big."), min_size)
        self.max_error_message = \
            max_error_message or lazy_inter(_("Make sure your uploaded file is at most %s bytes big."), max_size)
...
class MatchesRegularExpression:
    ...
    """
    def __init__(self, regexp, error_message=_("The format for this field is wrong.")):
...
class AnyValidator:
    ...
    """
    def __init__(self, validator_list=[], error_message=_("This field is invalid.")):
        self.validator_list = validator_list
        self.error_message = error_message
...
class URLMimeTypeCheck:
    ...
    def __call__(self, field_data, all_data):
        ...
        if content_type not in self.mime_type_list:
            raise URLMimeTypeCheck.InvalidContentType(
                _("The URL %(url)s returned the invalid Content-Type header '%(contenttype)s'.") % {
...
class RelaxNGCompact:
    ...
    def __call__(self, field_data, all_data):
        ...
        filename = mktemp()  # Insecure, but nothing else worked
        with open(filename, 'w') as fp:
            fp.write(field_data)
        if not path.exists(settings.JING_PATH):
            raise Exception("%s not found!" % settings.JING_PATH)
        p = popen('%s -c %s %s' % (settings.JING_PATH, self.schema_path, filename))
        ...
        for error in errors:
            ...
            if message.strip() == 'text not allowed here':
                display_errors.append(
                    _(
                        'Some text starting on line %(line)s is not allowed in that context. (Line starts with '
                        '"%(start)s".)'
                    ) % {
                        'line': line,
                        'start': lines[int(line) - 1][:30],
                    }
                )
                continue
            ...
            if message.strip() == 'required attributes missing':
                display_errors.append(
                    _(
                        'A tag on line %(line)s is missing one or more required attributes. (Line starts with '
                        '"%(start)s".)') % {
                            'line': line,
                            'start': lines[int(line) - 1][:30],
                        }
            ...
            if m:
                display_errors.append(
                    _(
                        'The "%(attr)s" attribute on line %(line)s has an invalid value. (Line starts with '
                        '"%(start)s".)') % {
                            'attr': m.group(1),
                            'line': line,
                            'start': lines[int(line) - 1][:30],
