...
class Subscription(models.Model):
    ...
    def bill_subscription(self, billing_date=None, dpp=10):
        """
        ...
        assert self.active, _('This subscription is not active and should not be billed.')

        # Check that the subscription is normal
        assert self.type in ('N', 'C'), _('This subscription is not normal or corporate and should not be billed.')

        # Check that the next billing date exists or we need to raise an exception.
        assert self.next_billing, _("Could not bill because next billing date does not exist")

        # Check that the subscription has a payment type
        assert self.payment_type, _("The subscription has no payment type, it can't be billed")
        ...
        if self.end_date:
            ...
            assert self.next_billing < self.end_date, error_msg

        if self.next_billing > billing_date + timedelta(billing_extra_days):
            raise Exception(_('This subscription should not be billed yet.'))

        # We need to get all the subscription data
        billing_data = self.get_billing_data_by_priority()

        if not billing_data and not force_dummy_missing_billing_data:
            raise Exception(
                f"Subscription {self.id} for contact {self.contact.id} contains no billing data."
            )

        if billing_data and billing_data["address"] is None:
            raise Exception(
                f"Subscription {self.id} for contact {self.contact.id} requires an address to be billed."
            )

        if billing_data and require_route_for_billing:
            if billing_data["route"] is None:
                raise Exception(
                    f"Subscription {self.id} for contact {self.contact.id} requires a route to be billed."
                )

            elif billing_data["route"] in exclude_routes_from_billing_list:
                raise Exception(
                    f"Subscription {self.id} for contact {self.contact.id} can't be billed since it's on"
                    f"route {billing_data['route']}."
                )

        product_summary = self.product_summary(with_pauses=True)

        # Calculate price and get invoice items
        amount, invoice_items = calc_price_from_products(
            product_summary,
            self.frequency,
            debug_id=f"subscription_{self.id}",
            create_items=True,
            subscription=self
        )

        # Handle envelope price if needed
        if envelope_price and SubscriptionProduct.objects.filter(subscription=self, has_envelope=1).exists():
            products_with_envelope_count = SubscriptionProduct.objects.filter(
                subscription=self, has_envelope=1
            ).count()
            envelope_amount = products_with_envelope_count * envelope_price * self.frequency
        ...
        service_from = self.next_billing

        try:
            # Create invoice if amount is greater than 0
            if amount > 0:
                invoice = Invoice.objects.create(
                    contact=self.get_billing_contact(),
                    payment_type=self.payment_type,
                    creation_date=billing_date,
                    service_from=service_from,
                    service_to=service_from + relativedelta(months=self.frequency),
                    ...
                    billing_document=self.get_billing_document(),
                    subscription=self,
                    amount=amount,
                )
            ...
            self.next_billing = (self.next_billing or self.start_date) + relativedelta(
            ...
            raise Exception(f"Contact {self.contact.id} Subscription {self.id}: {e}")
